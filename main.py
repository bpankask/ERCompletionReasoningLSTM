...
def collapseLabelMap(localMap, classes, roles, labels):
    """Basically combines the values from the classes, roles, and labels dictionaries into the localMap dictionary.
    LocalMap is used to find correct class or role which is then used to find correct label which is then assigned to localMap."""

    for mapping in localMap:  # Each mapping is a dictionary.
...
def getSnoDataFromFile(filename):  # I could mimic this for like a getRDFDataFromFile
    print("Loading data from " + filename)
    data = numpy.load(filename, allow_pickle=True)
...
if __name__ == "__main__":
    ...
    nTimesCrossValidate(n=args.cross, epochs=args.epochs, learningRate=args.learningRate, conceptSpace=10, roleSpace=8,
